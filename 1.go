...
func kendaraan_baru(j, n string, a, b, c, d, e, f int) kendaraan { 
 /*  Mengembalikan nilai kendaraan dengan data jenis kendaraan (j), nopol (n),jam masuk  
     dalam jam (a), menit (b), detik (c) dan jam keluar dalam jam (d), menit (e),  
     detik (f). */ 
	var k kendaraan 
	k.jke = j
	k.no = n
...
func bea_parkir(k *kendaraan) { 
 ...
 Proses: Memanggil prosedur durasi_parkir dan menghitung k.bea 
  Jika kendaraan mobil, biaya parkir jam pertama Rp 5000, per jam selanjutnya Rp  
  2500. Jika kendaraan motor, biaya parkir jam pertama Rp 2000, per jam selanjutnya  
  Rp 1000 
 FS: Durasi parkir (k.durasi) berisi nilai dalam satuan jam 
 */ 
	durasi_parkir(k)
